...
type SplitParams<
  S extends string,
  A extends string[] = [],
> = S extends `${infer F}&${infer R}` ? SplitParams<R, [...A, F]> : [...A, S];
type SetProperty<
  T extends object,
  K extends PropertyKey,
  V extends any = true,
> = {
  [P in keyof T | K as P extends "" ? never : P]: P extends K
    ? P extends keyof T
      ? T[P] extends V
        ? V
        : T[P] extends any[]
...
type MergeParams<S extends string[], R extends object = {}> = S extends [
  infer F,
  ...infer Rest extends string[],
]
  ? F extends `${infer K}=${infer V}`
    ? MergeParams<Rest, SetProperty<R, K, V>>
    : F extends `${infer K}=`
      ? MergeParams<Rest, SetProperty<R, K>>
      : MergeParams<Rest, SetProperty<R, F & string>>
  : R;
